...
def test(epoch):
    ...
    if not os.path.isdir('checkpoint'):
        os.mkdir('checkpoint')
    torch.save(state, './checkpoint/onecycle.pth')
    if acc > best_acc:
        best_acc = acc
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        checkpoint = torch.load('./checkpoint/onecycle.pth')
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.01, steps_per_epoch=128, epochs=200)
    summary(net, (3,32,32))
